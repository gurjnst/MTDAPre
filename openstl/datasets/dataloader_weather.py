...
mv_data_map = {
    **dict.fromkeys(['mv', 'mv4'], ['r', 't', 'u', 'v']),
    'mv5': ['g', 'r', 't', 'u', 'v'], 'mv5my': ['t2m', 'u10', 'v10', 'tcc', 'r'],
    'ms5my': ['t2m', 'u10', 'v10', 'tcc', 'r'],
    #'ms5my': ['tcc', 'u10', 'v10', 't2m', 'r'],
    #'ms5my': ['r', 'u10', 'v10', 't2m', 'tcc'],
}
...
class WeatherBenchDataset(Dataset):
    ...
    def __init__(self, data_root, data_name, training_time,
        ...
        if 'mv' in data_name:  # multiple input multiple output
        ...
        elif 'ms' in data_name:  # multiple input single output
            self.data_name = mv_data_map[data_name]
            self.data, self.mean, self.std = [], [], []
            for name in self.data_name:
                data, mean, std = self._load_data_xarray(data_name=name, single_variant=False)
    ...
    def __getitem__(self, index):
        ...
        labels = labels[:, :self.mean.shape[1], :, :]
        if self.use_augment:
...
if __name__ == '__main__':
    ...
    for _split in data_split:
        ...
        print(len(dataloader_train), len(dataloader_test)) # bs16-3284 1094 # bs1-52559 17495
        for item in dataloader_train:
            print(item[0].shape, item[1].shape) # [16, 12, 1, 32, 64] [16, 12, 1, 32, 64]
            break
