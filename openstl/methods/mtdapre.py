...
from openstl.models import mtdapre_c
from openstl.models import mtdapre_m
from openstl.models import mtdapre_t

from openstl.utils import reduce_tensor
from .base_method import Base_method


class MTDAPre(Base_method):
    ...
    def _build_model(self, args):
        if self.args.model == 'Early':
            return mtdapre_c.MTDAPre_Model(**args).to(self.device)
        elif self.args.model == 'Middle':
            return mtdapre_m.MTDAPre_Model(**args).to(self.device)
        elif self.args.model == 'Target':
            return mtdapre_t.MTDAPre_Model(**args).to(self.device)
