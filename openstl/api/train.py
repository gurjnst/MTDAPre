...
class BaseExperiment(object):
    ...
    def _preparation(self, dataloaders=None):
        ...
        if self.args.auto_resume:
            self.args.resume_from = osp.join(self.checkpoints_path, 'checkpoint.pth')
    ...
    def display_method_info(self):
        ...
        if self.args.method in ['simvp', 'tau', 'predformer', 'mtdapre']:
        ...
        elif self.args.method in ['convlstm', 'predrnnpp', 'predrnn', 'mim', 'e3dlstm', 'mau', 'motionrnn', 'gmg']:
    ...
    def train(self):
        ...
        for epoch in range(self._epoch, self._max_epochs):
            ...
            self._epoch = epoch
            if epoch >= 0 and epoch % self.args.log_step == 0:
                ...
                if self._rank == 0:
                    ...
                    if epoch >= 2000:
                        save_path = osp.join(self.path, f'epoch_{epoch+1}.pth')
                        torch.save(self.method.model.state_dict(), save_path)
            else:
                if self._rank == 0 and epoch % self.args.log_step == 0:
                    cur_lr = self.method.current_lr()
                    cur_lr = sum(cur_lr) / len(cur_lr)
                    print_log('Epoch: {0}, Steps: {1} | Lr: {2:.7f} | Train Loss: {3:.7f}\n'.format(
