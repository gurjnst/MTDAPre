...
def radar_csi(pred, true, threshold):
    pred = pred * 70
    true = true * 70

    pred_zeros = np.zeros(pred.shape)
    true_zeros = np.zeros(true.shape)

    pred_zeros[pred >= threshold] = 1
    true_zeros[true >= threshold] = 1
    csi_ = pred_zeros + true_zeros
...
def radar_hss(pred, true, threshold):
    pred = pred * 70
    true = true * 70
    ...
    return float(2 * (tp * tn - fp * fn)) / float(denom)
...
def metric(pred, true, mean=None, std=None, metrics=['mae', 'mse'],
    ...
    eval_res = {}
    eval_log = ""
    allowed_metrics = ['mae', 'mse', 'rmse', 'ssim', 'psnr', 'snr', 'lpips', 'csi30', 'csi40', 'hss30', 'hss40']
    ...
    if 'csi30' in metrics:
        eval_res['csi30'] = radar_csi(pred, true, threshold=30)

    if 'csi40' in metrics:
        eval_res['csi40'] = radar_csi(pred, true, threshold=40)

    if 'hss30' in metrics:
        eval_res['hss30'] = radar_hss(pred, true, threshold=30)

    if 'hss40' in metrics:
        eval_res['hss40'] = radar_hss(pred, true, threshold=40)
